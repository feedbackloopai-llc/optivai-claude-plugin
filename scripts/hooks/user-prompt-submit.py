...
def main():
    try:
        ...
        # Update Beads (auto-create beads for meaningful user prompts)
        try:
            from beads_writer import on_user_prompt as beads_on_user_prompt

            beads_on_user_prompt(
                prompt=user_prompt,
                session_id=session_id,
                project=logger.project_name,
                cwd=str(logger.project_dir)
            ...
            pass  # Beads module not available
        except Exception:
            pass  # Beads errors should not fail the hook

        # Update Open Brain (auto-capture trigger phrases)
        try:
            from brain_hook import on_user_prompt as brain_on_user_prompt

            brain_on_user_prompt(
                prompt=user_prompt,
                session_id=session_id,
                project=logger.project_name,
            )
        except ImportError:
            pass  # Brain hook not available
        except Exception:
            pass  # Brain errors should not fail the hook
