...
def _bootstrap_redact_pipeline():
    ...
    # Resolve scripts/ directory relative to this file's location so the import
    # works both from the live hooks location (~/.claude/hooks/) and from the
    # repo (scripts/hooks/memory_writer.py).
    _this_dir = _redact_os.path.dirname(_redact_os.path.abspath(__file__))
    # hooks/ lives one level below scripts/ in the repo; ~/.claude/hooks/ is flat
    # with a sibling redact/ package at ~/.claude/hooks/redact/.
    # Try the parent first (scripts/), then this dir itself.
    _candidates = [
