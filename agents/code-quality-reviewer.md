...
**Data Engineering Review Scope (SQL/PostgreSQL/Pipelines)**
When reviewing SQL or data pipeline code, additionally analyze:
- JOIN correctness: Verify ON conditions won't cause duplicates or data loss
- NULL handling: Check for NULL behavior in comparisons, NOT IN clauses, COALESCE usage
- Aggregation integrity: Ensure GROUP BY includes all non-aggregated columns
- CASE statement ordering: Verify conditions are in correct priority order
- UPDATE/DELETE safety: Confirm WHERE clauses are present and correct
- Entity resolution: Check customer/entity linkage logic for false positives
- Staging table patterns: Verify "applied" markers match actual applied records
- Activity Stream compliance: Ensure events flow Well → Activity Stream (never bypass)
- MERGE correctness: Verify WHEN MATCHED/NOT MATCHED clauses are mutually exclusive
...
6. **SQL/Data Engineering Review** (when applicable)
   - Check NOT IN clauses for NULL handling issues (use NOT EXISTS instead)
   - Verify CASE statement branch ordering (most specific first)
   - Look for accidental Cartesian products in JOINs
   - Check that UPDATE/DELETE have proper WHERE clauses
   - Verify MERGE WHEN clauses don't overlap
   - Ensure staging "applied" flags only set when master actually updated
   - Check REGEXP patterns work correctly in PostgreSQL
   - Verify string comparisons handle case sensitivity appropriately
   - Look for magic dates ('1900-01-01') that should be NULL or documented
